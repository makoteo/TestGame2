...
public class GamePanel extends JPanel implements Runnable, KeyListener{
	...
	private int CharHatCosts[] = {0, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000};
	...
	private void gameUpdate(){
		...
		if(gameState == STATE.Game && 
			...
			GamePanel.particles.add(new Particles(player.getx(), player.gety(), 1, 3, (int) (player.getr()*1.5)));
			
			//BULLET
			for(int i = 0; i < bullets.size(); i++){
				boolean remove = bullets.get(i).update();
				GamePanel.particles.add(new Particles((int) bullets.get(i).getx(), (int) bullets.get(i).gety(), 1, 4, (int) (bullets.get(i).getr()*1.5)));
				if(remove){
					bullets.remove(i);
					i--;
				}
    ...
    public int getCharHatCosts(int i){
    	return CharHatCosts[i];
    }
    public void setCharHatCosts(int i, int j){
		CharHatCosts[i] = j;
	}
